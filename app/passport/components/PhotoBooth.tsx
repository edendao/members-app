...
export const PhotoBooth: React.FC<PhotoBoothProps> = ({ size = 256, next, ...props }) => {
  ...
  return (
    <Stack direction={["column", null, "row"]} spacing={12} {...props}>
      <VStack align="start" spacing={6}>
        <Box animation="pulser-cw 10s ease-in-out infinite alternate" textAlign="left" w="100%">
          <Heading size="xs" mb={0}>
            mint your
          </Heading>
          <Shimmer size="md" pr={2} pt={1}>
            eden dao passport
          </Shimmer>
        </Box>
        <Typist cursor={{ show: false }} avgTypingDelay={50} key={3}>
          <Heading size="xs">the soulbound pass to the</Heading>
          <Typist.Delay ms={250} />
          <Shimmer size="sm" display="inline" pt={1}>
            eden dao omniverse.
          </Shimmer>
        </Typist>
      </VStack>
