...
interface SolarHeroProps extends StackProps {
  cta: string
  estimates: [Estimate, Estimate]
}

export const SolarHero: React.FC<SolarHeroProps> = ({ estimates, cta, ...stackProps }) => {
    ...
    >
      <VStack align="start" spacing={8} animation="pulser-ccw 5s ease-in-out infinite alternate">
        <Shimmer as="h1" size={["md", "lg"]} lineHeight={0.9}>
          your ultra-sound money could generate electricity!
        </Shimmer>
        <Text fontSize="lg">
          <Typist cursor={{ show: false }} avgTypingDelay={50}>
            ...
            of electricity. Over its lifetime, that number is closer to {total} TWh!
          </Typist>
        </Text>
