...
import { useVaultWithdraw } from "ds/hooks/web3/useVault"
...
export const Withdrawer: React.FC<WithdrawerProps> = ({ inputToken, outputToken }) => {
  ...
  const { write: withdraw, data: withdrawTransaction } = useVaultWithdraw(
    inputToken.address,
    inputAmount?.quotient?.toString() ?? "0",
    address,
    address
    ...
    <VStack spacing={6} p={[4, null, 8]} align="start">
      ...
      <VStack as={RadiantBackground} w="100%" p={8} spacing={5} borderRadius="2xl">
          ...
          <VStack>
            ...
            >
              <NumberInputField
                ...
                textAlign="right"
                autoFocus
                disabled={isApproveLoading || isWithdrawLoading}
              />
            </NumberInput>
          </VStack>
        ...
        <ButtonGroup size="lg" colorScheme="whiteAlpha" w="100%" spacing={6}>
          ...
          <Button
            flex={1}
            disabled={state === "approve" || isWithdrawLoading}
            onClick={() => withdraw()}
